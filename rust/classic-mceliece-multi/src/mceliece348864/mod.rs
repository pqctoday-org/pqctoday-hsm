//! Classic McEliece 348864 — NIST category 1, narrow field (GFBITS=12), non-`f`
//! (fully systematic keygen). Smallest of the 10 parameter sets.
...
/// Verified (P0-3 / the KAT README table) against the official Round-4 vectors.
pub const CRYPTO_PUBLICKEYBYTES: usize = 261_120;
/// Upstream literal (`api.rs`) — never derived from GFBITS/SYS_N/SYS_T.
...
/// KEM keypair generation. The public key is meant to be shared with any party;
/// access to the secret key must be limited to the generating party.
...
/// KEM Encapsulation. Given a public key, compute a shared key and the ciphertext
/// that should be sent back to the holder of the corresponding secret key.
...
/// KEM Decapsulation. Given a secret key and a ciphertext, determine the shared key
...
mod tests {
    ...
    fn sizes_match_the_official_table() {
        ...
        assert_eq!(CRYPTO_PUBLICKEYBYTES, 261_120);
        assert_eq!(CRYPTO_SECRETKEYBYTES, 6_492);
        assert_eq!(CRYPTO_CIPHERTEXTBYTES, 96);
    }

    #[test]
    #[ignore = "348864 keygen is slow in debug builds — matches the upstream crate's own [profile.dev] opt-level note; run --release"]

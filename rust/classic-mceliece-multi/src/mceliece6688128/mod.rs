//! Classic McEliece 6688128 — NIST category 5, wide field (GFBITS=13), non-`f`.
//! BSI TR-02102-1 §2.4.2 recommended. Previously the only variant this engine
//! shipped (`softhsmrustv3` v0.7.0, single-feature build) — the historic
//! `CKP_CLASSIC_MCELIECE_6688128 = 0x1` PKCS#11 value stays pinned to this variant.
...
pub const CRYPTO_PUBLICKEYBYTES: usize = 1_044_992;
pub const CRYPTO_SECRETKEYBYTES: usize = 13_932;
...
mod tests {
    ...
    fn sizes_match_the_official_table() {
        ...
        // Matches the engine's pre-existing single-variant constant
        // (rust/src/native/keygen.rs's CRYPTO_PUBLICKEYBYTES/CRYPTO_SECRETKEYBYTES
        // checks against classic-mceliece-rust v2.0.2 for this exact variant).
        assert_eq!(CRYPTO_PUBLICKEYBYTES, 1_044_992);
        assert_eq!(CRYPTO_SECRETKEYBYTES, 13_932);
    }

    #[test]
    #[ignore = "6688128 keygen is minutes-slow in debug builds — see the engine's own doc comment; run --release"]

//! Classic McEliece 8192128 — NIST category 5, wide field (GFBITS=13), non-`f`.
//! `SYS_N == 1<<GFBITS` (8192) for this size alone, so `gen_e` needs no
//! range-reject-and-retry filter (`encrypt::gen_e_direct`, `encrypt_direct_plain`)
//! — every sampled index is already in range. BSI TR-02102-1 §2.4.2 recommended;
//! largest of the 10 parameter sets (1,357,824-byte public key).
...
mod tests {
    ...
    fn sizes_match_the_official_table() {
        ...
        assert_eq!(SYS_N, 1 << GFBITS); // defines this variant's gen_e-direct path
    }

    #[test]
    #[ignore = "8192128 keygen is minutes-slow in debug builds — run --release"]

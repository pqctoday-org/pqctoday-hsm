//! Classic McEliece 6960119 — NIST category 5, wide field (GFBITS=13), non-`f`.
//! `PK_NROWS` (1547) is not a multiple of 8, so this variant alone needs the
//! bit-realigned `syndrome`/`pk` writeback (`_padded`/`_6960119` bodies throughout
//! `shared/`) neither ISO nor BSI's other three large sizes require.
...
pub fn keypair_boxed<R: CryptoRng + RngCore>(rng: &mut R) -> (PublicKeyOwned, SecretKeyOwned) {
    ...
    operations::crypto_kem_keypair_6960119(
        pk.as_mut(),
        sk.as_mut(),
        rng,
...
mod tests {
    ...
    fn sizes_match_the_official_table() {
        ...
        assert_eq!(PK_NROWS % 8, 3); // the non-byte-aligned PK_NROWS this variant is defined by
    }

    #[test]
    #[ignore = "6960119 keygen is minutes-slow in debug builds — run --release"]

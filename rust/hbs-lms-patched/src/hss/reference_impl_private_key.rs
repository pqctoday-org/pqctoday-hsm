...
impl CompressedParameterSet {
    ...
    pub fn from<H: HashChain>(parameters: &[HssParameter<H>]) -> Result<Self, ()> {
        ...
        for (i, parameter) in parameters.iter().enumerate() {
            ...
            // Store CANONICAL variant discriminants (LMS H5..H25 → 5..9,
            // LM-OTS W1..W8 → 1..4) rather than the family-specific wire
            // type IDs: SHAKE ids (LMS up to 0x18) overflow the nibble, and
            // the hash family is re-derived from `H` at decode time. This
            // matches the historical on-disk format for SHA-256 keys.
            let lmots_type = match lmots.get_type_id().checked_sub(H::LMOTS_TYPE_BASE) {
                Some(off) => (off + 1) as u8,
                None => lmots.get_type_id() as u8, // ids below the family base (test-only)
            };
            let lms_type = match lms.get_type_id().checked_sub(H::LMS_TYPE_BASE) {
                Some(off) => (off + 5) as u8,
                None => lms.get_type_id() as u8, // ids below the family base (test-only H2)
            };

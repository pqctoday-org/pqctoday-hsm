...
use crate::util::helper::{try_read, try_read_and_advance};
...
impl<'a, H: HashChain> InMemoryLmsSignature<'a, H> {
    pub fn new(data: &'a [u8]) -> Option<Self> {
        // Parsing like 5.4.2 Algorithm 6a. Bounds-checked: signature bytes are
        // caller-controlled (corrupt vectors must yield None, not a panic).
        let mut index = 0;

        let lms_leaf_identifier =
            u32::from_be_bytes(try_read_and_advance(data, 4, &mut index)?.try_into().ok()?);
        ...
        let lmots_parameter = LmotsAlgorithm::get_from_type::<H>(u32::from_be_bytes(
            try_read(data, 4, &index)?.try_into().ok()?,
        ))?;
        let lmots_signature = lm_ots::signing::InMemoryLmotsSignature::new(try_read_and_advance(
            data,
            (4 + H::OUTPUT_SIZE * (1 + lmots_parameter.get_hash_chain_count())) as usize,
            &mut index,
        )?)?;

        let _type = u32::from_be_bytes(try_read_and_advance(data, 4, &mut index)?.try_into().ok()?);

        let lms_parameter = LmsAlgorithm::get_from_type(_type)?;
        let authentication_path = try_read_and_advance(
            data,

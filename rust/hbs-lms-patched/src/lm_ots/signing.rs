use crate::{
    ...
    util::{coef::coef, helper::try_read_and_advance},
};
...
impl<'a, H: HashChain> InMemoryLmotsSignature<'a, H> {
    pub fn new(data: &'a [u8]) -> Option<Self> {
        // Bounds-checked parse: wire bytes are caller-controlled.
        let mut index = 0;

        let lmots_parameter = LmotsAlgorithm::get_from_type::<H>(u32::from_be_bytes(
            try_read_and_advance(data, 4, &mut index)?.try_into().ok()?,
        ))?;

        let signature_randomizer = try_read_and_advance(data, H::OUTPUT_SIZE as usize, &mut index)?;

        let signature_data = try_read_and_advance(
            data,

...
impl From<u32> for LmotsAlgorithm {
    /// Family-agnostic: maps any SP 800-208 LM-OTS type ID (SHA-256 N32
    /// 0x01-0x04, SHA-256 N24 0x05-0x08, SHAKE N32 0x09-0x0C, SHAKE N24
    /// 0x0D-0x10) to its Winternitz variant.
    fn from(_type: u32) -> Self {
        match _type {
            0x01..=0x10 => match (_type - 0x01) % 4 {
                0 => LmotsAlgorithm::LmotsW1,
                1 => LmotsAlgorithm::LmotsW2,
                2 => LmotsAlgorithm::LmotsW4,
                _ => LmotsAlgorithm::LmotsW8,
            },
...
impl LmotsAlgorithm {
    ...
    pub fn get_from_type<H: HashChain>(_type: u32) -> Option<LmotsParameter<H>> {
        // Accept only this hash family's IANA range (SP 800-208 §4).
        match _type.checked_sub(H::LMOTS_TYPE_BASE) {
            Some(0) => LmotsAlgorithm::LmotsW1.construct_parameter(),
            Some(1) => LmotsAlgorithm::LmotsW2.construct_parameter(),
            Some(2) => LmotsAlgorithm::LmotsW4.construct_parameter(),
            Some(3) => LmotsAlgorithm::LmotsW8.construct_parameter(),
            _ => None,
        }
    }
}

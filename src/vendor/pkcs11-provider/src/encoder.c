...
/* X25519/X448 URI-PEM PrivateKeyInfo encoder (remediation R16). Modeled
 * directly on ec_edwards's block just above — same generic
 * p11prov_encoder_private_key_write_pem helper, just CKK_EC_MONTGOMERY
 * instead of CKK_EC_EDWARDS (these are genuinely distinct PKCS#11 key
 * types even though both live under the "ec" umbrella: Edwards is
 * EdDSA/signing, Montgomery is X25519/X448/key-exchange). One shared
 * function/table for both curves, same as every other multi-variant
 * block in this file — the montgomery keymgmt's own OSSL_PARAM already
 * carries which curve. Never touches the private key bytes, same
 * PEM-wraps-a-pkcs11:-URI pattern as every other encoder here. */
static int p11prov_montgomery_encoder_priv_key_info_pem_encode(
    void *inctx, OSSL_CORE_BIO *cbio, const void *inkey,
    const OSSL_PARAM key_abstract[], int selection,
    OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg)
{
    return p11prov_encoder_private_key_write_pem(
        CKK_EC_MONTGOMERY, inctx, cbio, inkey, key_abstract, selection, cb,
        cbarg);
}

const OSSL_DISPATCH p11prov_montgomery_encoder_priv_key_info_pem_functions[] = {
    DISPATCH_BASE_ENCODER_ELEM(NEWCTX, newctx),
    DISPATCH_BASE_ENCODER_ELEM(FREECTX, freectx),
    DISPATCH_ENCODER_ELEM(DOES_SELECTION, common, priv_key_info, pem,
                          does_selection),
    DISPATCH_ENCODER_ELEM(ENCODE, montgomery, priv_key_info, pem, encode),
    { 0, NULL },
};
...
/* ML-KEM SPKI + text encoders (remediation R16 encoder-parity tier),
 * modeled directly on the ML-DSA block above — same shape, same
 * p11prov_obj_export_public_key callback pattern, same NID switch on
 * CKA_PARAMETER_SET. One shared function/table across all 3 parameter
 * sets, same as the priv_key_info encoder just above. */
...
static int p11prov_slhdsa_set_keypoint_data(const OSSL_PARAM *params,
                                            void *key)
{
...
static X509_PUBKEY *p11prov_slhdsa_pubkey_to_x509(P11PROV_OBJ *key)
{
#ifdef NID_SLH_DSA_SHA2_128s
    ...
    ret = p11prov_obj_export_public_key(key, CKK_SLH_DSA, true, false,
                                        p11prov_slhdsa_set_keypoint_data,
                                        &keypoint);
    if (ret != RET_OSSL_OK) {
        OPENSSL_clear_free(keypoint.octet, keypoint.len);
        return NULL;
...
static int p11prov_slhdsa_encoder_spki_der_encode(
    void *inctx, OSSL_CORE_BIO *cbio, const void *inkey,
    const OSSL_PARAM key_abstract[], int selection,
    OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg)
{
    ...
    P11PROV_debug("slhdsa SubjectPublicKeyInfo DER Encoder");
    ...
    pubkey = p11prov_slhdsa_pubkey_to_x509(key);
    if (!pubkey) {
        ret = RET_OSSL_ERR;
        goto done;
    }
...
const OSSL_DISPATCH p11prov_slhdsa_encoder_spki_der_functions[] = {
    ...
    DISPATCH_ENCODER_ELEM(ENCODE, slhdsa, spki, der, encode),
    { 0, NULL },
};

static int p11prov_slhdsa_print_public_key(const OSSL_PARAM *params,
                                            void *bio)
{
    BIO *out = (BIO *)bio;
    const OSSL_PARAM *p;
...
/* Remediation R41 (phase 8): XMSS/XMSS^MT. URI-PEM PrivateKeyInfo only —
 * no SPKI/OID story exists for either in X.509 (OpenSSL 3.6 has no XMSS
 * names/OIDs/verify support at all, unlike LMS — see the phase-8 plan's
 * own R41 grounding), so unlike ML-DSA/ML-KEM there is no SPKI-DER/text
 * encoder pair to add here. Same generic shared-core pattern as every
 * other type above. */
static int p11prov_xmss_encoder_priv_key_info_pem_encode(
    void *inctx, OSSL_CORE_BIO *cbio, const void *inkey,
    const OSSL_PARAM key_abstract[], int selection,
    OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg)
{
    return p11prov_encoder_private_key_write_pem(
        CKK_XMSS, inctx, cbio, inkey, key_abstract, selection, cb, cbarg);
}

const OSSL_DISPATCH p11prov_xmss_encoder_priv_key_info_pem_functions[] = {
    DISPATCH_BASE_ENCODER_ELEM(NEWCTX, newctx),
    DISPATCH_BASE_ENCODER_ELEM(FREECTX, freectx),
    DISPATCH_ENCODER_ELEM(DOES_SELECTION, common, priv_key_info, pem,
                          does_selection),
    DISPATCH_ENCODER_ELEM(ENCODE, xmss, priv_key_info, pem, encode),
    { 0, NULL },
};

static int p11prov_xmssmt_encoder_priv_key_info_pem_encode(
    void *inctx, OSSL_CORE_BIO *cbio, const void *inkey,
    const OSSL_PARAM key_abstract[], int selection,
    OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg)
{
    return p11prov_encoder_private_key_write_pem(
        CKK_XMSSMT, inctx, cbio, inkey, key_abstract, selection, cb, cbarg);
}

const OSSL_DISPATCH p11prov_xmssmt_encoder_priv_key_info_pem_functions[] = {
    DISPATCH_BASE_ENCODER_ELEM(NEWCTX, newctx),
    DISPATCH_BASE_ENCODER_ELEM(FREECTX, freectx),
    DISPATCH_ENCODER_ELEM(DOES_SELECTION, common, priv_key_info, pem,
                          does_selection),
    DISPATCH_ENCODER_ELEM(ENCODE, xmssmt, priv_key_info, pem, encode),
    { 0, NULL },
};
                                              ...
                                              void *cbarg)
{
    ...
    CK_OBJECT_CLASS class;
    CK_ULONG param_set;
    const char *type_name = "SLH-DSA";
    const char *uri = NULL;
    BIO *out;
    int ret;

    P11PROV_debug("slhdsa Text Encoder");
    ...
    switch (param_set) {
    case CKP_SLH_DSA_SHA2_128S:
        type_name = "SLH-DSA-SHA2-128s";
        break;
    case CKP_SLH_DSA_SHAKE_128S:
        type_name = "SLH-DSA-SHAKE-128s";
        break;
    case CKP_SLH_DSA_SHA2_128F:
        type_name = "SLH-DSA-SHA2-128f";
        break;
    case CKP_SLH_DSA_SHAKE_128F:
        type_name = "SLH-DSA-SHAKE-128f";
        break;
    case CKP_SLH_DSA_SHA2_192S:
        type_name = "SLH-DSA-SHA2-192s";
        break;
    case CKP_SLH_DSA_SHAKE_192S:
        type_name = "SLH-DSA-SHAKE-192s";
        break;
    case CKP_SLH_DSA_SHA2_192F:
        type_name = "SLH-DSA-SHA2-192f";
        break;
    case CKP_SLH_DSA_SHAKE_192F:
        type_name = "SLH-DSA-SHAKE-192f";
        break;
    case CKP_SLH_DSA_SHA2_256S:
        type_name = "SLH-DSA-SHA2-256s";
        break;
    case CKP_SLH_DSA_SHAKE_256S:
        type_name = "SLH-DSA-SHAKE-256s";
        break;
    case CKP_SLH_DSA_SHA2_256F:
        type_name = "SLH-DSA-SHA2-256f";
        break;
    case CKP_SLH_DSA_SHAKE_256F:
        type_name = "SLH-DSA-SHAKE-256f";
        break;
    default:
        BIO_printf(out, "[Error: Key Parameter set unknown %ld]", param_set);
    }
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) {
        ...
        BIO_printf(out, "PKCS11 %s Public Key\n", type_name);
        ret = p11prov_obj_export_public_key(
            key, CKK_SLH_DSA, true, false, p11prov_slhdsa_print_public_key,
            out);
        if (ret != RET_OSSL_OK) {
            BIO_printf(out, "[Error: Failed to decode public key data]\n");
        }
...
const OSSL_DISPATCH p11prov_slhdsa_encoder_text_functions[] = {
    DISPATCH_BASE_ENCODER_ELEM(NEWCTX, newctx),
    DISPATCH_BASE_ENCODER_ELEM(FREECTX, freectx),
    DISPATCH_TEXT_ENCODER_ELEM(ENCODE, slhdsa, encode_text),
    { 0, NULL },
};

                             ...
                             CK_ULONG_PTR pulCiphertextLen, CK_OBJECT_HANDLE_PTR phKey)
{
    ...
    if (p11prov_ctx_is_call_blocked(ctx, P11PROV_BLOCK_EncapsulateKey)) {
        P11PROV_debug("C_%s is blocked", "EncapsulateKey");
        return CKR_FUNCTION_NOT_SUPPORTED;
    }
    if (!intf->EncapsulateKey) {
        P11PROV_debug("C_%s is not available", "EncapsulateKey");
        return CKR_FUNCTION_NOT_SUPPORTED;
    }
    P11PROV_debug("Calling C_EncapsulateKey");
    ret = intf->EncapsulateKey(hSession, pMechanism, hKey, pTemplate, ulAttributeCount, pCiphertext, pulCiphertextLen, phKey);
    if (ret != CKR_OK) {
        P11PROV_debug("Error %ld returned by C_EncapsulateKey", ret);
    }
    return ret;
}
                             ...
                             CK_ULONG ulCiphertextLen, CK_OBJECT_HANDLE_PTR phKey)
{
    ...
    if (p11prov_ctx_is_call_blocked(ctx, P11PROV_BLOCK_DecapsulateKey)) {
        P11PROV_debug("C_%s is blocked", "DecapsulateKey");
        return CKR_FUNCTION_NOT_SUPPORTED;
    }
    if (!intf->DecapsulateKey) {
        P11PROV_debug("C_%s is not available", "DecapsulateKey");
        return CKR_FUNCTION_NOT_SUPPORTED;
    }
    P11PROV_debug("Calling C_DecapsulateKey");
    ret = intf->DecapsulateKey(hSession, pMechanism, hKey, pTemplate, ulAttributeCount, pCiphertext, ulCiphertextLen, phKey);
    if (ret != CKR_OK) {
        P11PROV_debug("Error %ld returned by C_DecapsulateKey", ret);
    }
    return ret;
}

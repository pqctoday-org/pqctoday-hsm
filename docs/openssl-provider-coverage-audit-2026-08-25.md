...
| OP-3 | **Core RESOLVED (R3, 2026-08-25)** — was: ML-KEM had zero encoders registered, so `genpkey -algorithm ML-KEM-768 -out k.pem` generated and persisted a real key on-token (R3b) but the `-out` write step failed, `Error writing key(s)`/exit 1. **Correction to this row's own earlier wording** (found live during R3, not assumed): public-key output was never actually broken — `storeutl -text`/`pkey -pubout` already worked with zero encoders, because ML-KEM's keymgmt EXPORT function bridges the public bytes into OpenSSL's default provider, which encodes them. The real, sole functional gap was the **private**-key URI-PEM PrivateKeyInfo encoder — the one path that can't use that bridge (private material never crosses into another provider). Fixed: `p11prov_mlkem_encoder_priv_key_info_pem_encode` (`encoder.c`), registered for all 3 variants inside the `encode_pkey_as_pk11_uri` block (`provider.c`). Like every other PrivateKeyInfo encoder in this fork, it never touches raw key bytes — `p11prov_encoder_private_key_to_asn1` calls `p11prov_obj_get_public_uri(key)` and PEM-wraps that `pkcs11:` URI string; live-verified the written file decodes to a `type=private` URI, and a negative harness assertion checks no `PRIVATE KEY` label ever appears. **Parity tier RESOLVED (R40, phase 8, 2026-08-26)** — was tracked here as an open follow-up (R33): live-checking before writing new code found the SPKI-DER + text encoder pair already shipped, phase 3 (R16) — `p11prov_mlkem_encoder_spki_der_*`/`p11prov_mlkem_encoder_text_functions`, registered, harness `T4x_spki` already green. What R16's own narrative had actually left unresolved (never separately sabotage-tested whether the dedicated SPKI encoder is genuinely reachable) is what R40 closed: engine-log-confirmed neither ML-KEM's nor ML-DSA's dedicated SPKI-DER encoder is ever reached by any CLI path (OpenSSL core's own generic keymgmt-export bridge always wins) — a new permanent, documented limitation, not a registration bug. See this doc's own "Phase 8, R40" entry below and harness `T4x_spki_noexport`. | `encoder.c`; live (T4x_encode, T4x_spki, T4x_spki_noexport, T10 as network-effect control) | ~~Medium~~ — |
...
| ALG-2 | **RESOLVED (R41, phase 8, 2026-08-26)** — was: `sig/xmss.c` a stub with empty `OSSL_DISPATCH` tables, unreachable, but already wired into `provider.c`'s registration (advertise-without-dispatch). Now: real `XMSS`/`XMSSMT` keymgmt + `sig/xmss.c` (mirroring `sig/hss.c`'s accumulate-then-single-`C_Sign` shape), single algorithm name per family (matching HSS's own "one name, one hardcoded default" precedent — no dynamic per-`OSSL_PARAM` paramset exists anywhere in this codebase to mirror ML-KEM's per-variant pattern instead). Two registration gaps found live and fixed beyond the originally-scoped work: `provider.c`'s `PQC_MECHS` checklist never included `CKM_XMSS`/`CKM_XMSSMT` (the written `case` arms were dead code), and `store.c`'s naming switch had no `CKK_XMSS`/`CKK_XMSSMT` cases. Live-verified both param-set defaults (XMSS-SHA2_10_256 = 2500-byte sig, XMSSMT-SHA2_20/2_256 = 4963-byte sig, matching the ported RFC 8391 formulas exactly), both sabotage controls, a genuine multi-process leaf-index-advance proof (idx 0→1, first signature still verifies after the second), and a Rust-arm engine-agnostic-dispatch smoke test — harness T32/T32b/T32c/T32d. No RFC 8391/SP 800-208 KAT vectors or from-scratch independent verifier (OpenSSL ships no native XMSS implementation to check against, unlike LMS) — deferred future work, not a permanent limitation, see this doc's own "Phase 8, R41" entry below. | both engines **sign+verify** | ~~`sig/xmss.c` stub, unreachable; no native OpenSSL names exist (custom names required; no CMS/TLS story)~~ — | ~~Medium~~ — |
...
**Phase 8, R41 (XMSS/XMSS-MT provider surface, un-parks R27, closes
ALG-2), DONE — the last algorithm-family gap in the matrix, and the
last phase-8 item.** `sig/xmss.c` was a 20-line stub with empty
`OSSL_DISPATCH` tables that was, nonetheless, already wired into
`provider.c`'s registration — meaning pre-R41, this provider
advertised a usable "XMSS" algorithm while every real operation
failed outright (advertise-without-dispatch, same class as G3).
Rewritten as a full ~430-line implementation mirroring `sig/hss.c`'s
accumulate-then-single-`C_Sign` stateful shape, with signature-size
formulas ported from Rust's own `get_sig_len` XMSS/XMSS-MT arms.

**Design decision made during execution**: single algorithm name per
family (`XMSS`, `XMSSMT`), not one name per RFC 8391 parameter set.
The plan's own grounding suggested mirroring ML-KEM's per-variant
pattern, but live investigation of `keymgmt.c`'s `struct
key_generator` union found no existing precedent anywhere in this
codebase for a keymgmt reading a dynamic parameter set from a caller
`OSSL_PARAM` — even ML-DSA's own `param_set` is hardcoded per wrapper
function. `sig/hss.c`'s own "one name, one hardcoded default" choice
is the actual closest precedent, adopted here too (default
`CKP_XMSS_SHA2_10_256` / `CKP_XMSSMT_SHA2_20/2_256`); a non-default
parameter set stays reachable via a raw `C_GenerateKeyPair` template,
same escape hatch HSS already documents.

**Two gaps neither the plan's own grounding nor the R1/R4 "five-gap
checklist" it cited actually named, both found by live smoke-testing
rather than static review**: `provider.c`'s `PQC_MECHS` checklist
macro — the per-mechanism registration loop's own allow-list — had
never been updated for `CKM_XMSS`/`CKM_XMSSMT` (and their
`_KEY_PAIR_GEN` twins), so the `case CKM_XMSS:`/`case CKM_XMSSMT:`
arms already written into the registration switch were dead code: the
loop's own membership test never matched, so the signature
algorithm's `ADD_ALGO_EXT` never ran, even though both engines
correctly advertise the mechanisms via `C_GetMechanismInfo`. Because
keymgmt registers unconditionally (outside this loop), `genpkey
-algorithm XMSS` worked regardless — which is exactly what let this
hide behind a passing keygen smoke test, surfacing only as OpenSSL's
own generic `operation not supported for this keytype` (no
XMSS-specific diagnostic at all) on `pkeyutl -sign`, found via
`PKCS11_PROVIDER_DEBUG` tracing. Separately, `store.c`'s own
`CKO_PRIVATE_KEY`/`CKO_PUBLIC_KEY` → algorithm-name switch (used for
`pkcs11:` URI object loading) had no `CKK_XMSS`/`CKK_XMSSMT` cases —
this, not `objects.c` (whose own analogous caching-skip guard R29 had
already added pre-emptively), is where the plan's own step-3 "does
`store.c` have its own gap" question actually resolved yes.
`objects.c`'s `fetch_xmss_key` (mirroring `fetch_hss_key`) was also
genuinely missing, and so was the URI-PEM `PrivateKeyInfo` encoder —
each live-reproduced as a distinct failure (`Unsupported key type
(71)`, then `Could not find private key…`, then `No encoders were
found`) and fixed in sequence rather than found all at once by
reading.

**Proof plan delivered with one deliberate, documented scope cut.**
Shipped: sabotage twins (tampered signature + wrong message) for both
XMSS and XMSS-MT, both `-rawin` and plain dispatch (T32/T32b); a
multi-process stateful-counter proof (T32d) — the RFC 8391 §4.1.9
leaf index `idx` (leading 4 bytes of the signature) genuinely advances
0→1 across two wholly separate `pkeyutl -sign` processes sharing only
the C++ engine's own on-disk token store, no `SOFTHSMRUST_STATE_FILE`-
style bridge needed for this arm (unlike HSS's Rust-arm proof, T24e —
the C++ engine persists natively); a Rust-arm engine-agnostic-dispatch
smoke test (T32c) confirming the identical provider code sign/verifies
correctly against the Rust engine's own independent XMSS
implementation too. **Cut**: RFC 8391/SP 800-208 KAT vectors and a
from-scratch independent verifier (HSS/LMS's own `lms_xdr_verify`
precedent works only because OpenSSL 3.6.3 ships a native LMS
implementation to check against — there is no native XMSS
implementation to build an equivalent oracle from, so closing this
gap for real means writing a full RFC 8391 WOTS+/L-tree/tree verifier
from scratch, not just sourcing a KAT file). Documented as deferred
future work in the plan doc's own R41 execution update, not folded
into the permanent-limitations list — unlike that list's five items,
this one is genuinely closeable, just not attempted in this item.

Full regression: **harness 89/89** (4 new cases: T32, T32b, T32c,
T32d — zero regressions), **C++ CTest 8/8**. No Rust change — every
engine-side prerequisite (keygen, sign, verify, sig-size formulas)
pre-existed this item; only the provider's registration/dispatch
surface needed building.

This closes phase 8's R41 — and with it every phase-8 item (R37–R41).
R27 (un-parked by this item) and R33 (un-parked by R40) both close;
only R33's own already-tracked follow-on and the five documented
permanent limitations remain open across the whole provider.

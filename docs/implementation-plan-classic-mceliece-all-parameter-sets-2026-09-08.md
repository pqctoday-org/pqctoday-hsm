...
| P0-2 | **liboqs build spike**: submodule at `0.16.0`, minimal McEliece-only build against OpenSSL 3.6.3 in the `pqc-rust` container and on the macOS host; confirm the exact `OQS_MINIMAL_BUILD` identifiers and that `OQS_ENABLE_KEM_classic_mceliece_*` all resolve. | `liboqs` links into a throwaway target; `OQS_KEM_alg_count()` shows exactly ten. |
| P0-3 | **KAT sourcing**: locate the official round-4 KAT files per set (NIST submission package / classic.mceliece.org), record sha256 + URL, confirm the `.rsp` format matches what `test_katkem.rs` parses. | Files + provenance README in `kmip/kat/classic-mceliece/`, nothing generated locally. |

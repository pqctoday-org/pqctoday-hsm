...
pub fn register(
        ...
        (key_material.as_ref(), key_format_type, deps.engine_session)
    {
        match (req.object_type, kmip_algorithm) {
            ...
            // WP-R/R2 (cert-ops plan revision) — ECDSA/Ed25519 PublicKey
            // import. The engine has `register_ecdsa_public_key`/
            // `register_ed25519_public_key` since WP1a (real `spki`/`der`
            // decode), but nothing in this Register handler ever called
            // them — a client could not Register an externally-generated
            // EC/Ed25519 public key at all before this. Same DER-format
            // convention as the RSA arm above: PKCS_8 (4) and X_509 (5)
            // are both real SubjectPublicKeyInfo DER (X_509 is the
            // canonical name for it; PKCS_8 is KMIP's public-key reuse of
            // the private-key format name for the same DER shape); PKCS_1
            // (3) is RSA-specific ASN.1 and does not apply here.
            (ObjectType::PublicKey, alg @ (crate::kmip30::KmipAlgorithm::Ecdsa
                | crate::kmip30::KmipAlgorithm::Ed25519)) =>
            {
                if !matches!(fmt, 4 | 5) {
                    return Err(fail_err(deps, correlation_id, "Register",
                        KmipError::key_format_type_not_supported(format!(
                            "{} PublicKey Register: supports PKCS_8 (0x04) / X_509 (0x05) \
                             SubjectPublicKeyInfo DER only, got 0x{fmt:02x}",
                            canonical_name(alg),
                        ))));
                }
                let import = if alg == crate::kmip30::KmipAlgorithm::Ecdsa {
                ...
                import.map_err(|rv| fail_err(deps, correlation_id, "Register",
                    super::helpers::ck_rv_to_kmip_error(rv, "Register:ec-public-import")))?;
            }
...
mod tests {
    ...
    /// WP-R/R2 (cert-ops plan revision) — Register a GENUINELY external
    /// ECDSA P-256 SubjectPublicKeyInfo (rcgen's own aws-lc-rs-backed
    /// keygen, a dev-dependency already used elsewhere in this crate as
    /// an independent fixture generator — not this engine's own output),
    /// confirm it round-trips byte-identical through Get, then Certify it
    /// by UID. Closes the gap WP1a opened but never wired into Register:
    /// before this, nothing in the Register handler ever called
    /// `register_ecdsa_public_key`, so an externally-generated EC key
    /// could not be Registered at all.
    #[test]
    fn register_external_ecdsa_public_key_round_trips_and_certifies() {
        use softhsmrustv3::native::session;

        let _g = crate::ops::helpers::engine_lock();
        let _ = session::finalize();
        session::init().expect("engine init");
        let sess = session::bootstrap_default_token(0, "so-pin", "user-pin", "wpr2-register-ec")
            .expect("bootstrap session");

        // A real, independent ECDSA P-256 keypair — NOT generated by this
        // engine. `public_key_der()` is rcgen's own real SPKI DER export.
        let external_kp = rcgen::KeyPair::generate().expect("rcgen keypair");
        let external_spki = external_kp.public_key_der();
        ...
        let resp = register(&deps, RegisterRequest {
            secret_data_type: None,
            object_type: ObjectType::PublicKey,
            attributes: vec![Attribute::CryptographicUsageMask(UsageMask::VERIFY)],
            managed_object: Some(KeyBlock {
                key_format_type: KeyFormatType::X509,
                cryptographic_algorithm: KmipAlgorithm::Ecdsa,
                cryptographic_length: 256,
                key_value: external_spki.clone(),
                key_wrapping_data: None,
            }),
            protection_storage_masks: None,
            certificate_payload: None,
        ...
        // Now certify the Registered key by UID — the R2 half of the
        // WP-R scenario (R1 covers the CreateKeyPair-without-Register
        // case; this covers "a key from elsewhere").
        let ca = crate::ops::certify::tests::bootstrap_ca(KmipAlgorithm::Ecdsa);
        // Reuse the SAME engine session for the CA fixture and the
        // Registered key — bootstrap_ca creates its own session, so
        // certify against ITS deps instead of re-registering there.
        let reregistered = register(&ca.deps, RegisterRequest {
            secret_data_type: None,
            object_type: ObjectType::PublicKey,
            attributes: vec![Attribute::CryptographicUsageMask(UsageMask::VERIFY)],
            managed_object: Some(KeyBlock {
        ...
        assert_eq!(cert_rec.object_type, ObjectType::Certificate);

        let _ = session::finalize();
    }

    /// Negative: malformed DER (correctly labeled X_509, but garbage
    /// content) must fail Register, not silently produce no engine
    /// object. `InvalidField` (not `KeyFormatTypeNotSupported`) is the
    /// correct code here — the client's chosen KeyFormatType is fine;
    /// the BYTES are what's wrong. Matches the RSA public-key arm's
    /// existing convention: both route a decode failure through the
    /// engine's own `CKR_KEY_TYPE_INCONSISTENT` -> `InvalidField`
    /// mapping (`ops::helpers::ck_rv_to_kmip_error`), not a hand-rolled
    /// second error path.
    #[test]
    fn register_ecdsa_public_key_malformed_der_fails_invalid_field() {
        use softhsmrustv3::native::session;

        let _g = crate::ops::helpers::engine_lock();
        let _ = session::finalize();
        session::init().expect("engine init");
        let sess = session::bootstrap_default_token(0, "so-pin", "user-pin", "wpr2-register-ec-neg")
            .expect("bootstrap session");
        let ring = Arc::new(RingSink::new(64));
        let sink: Arc<dyn AuditSink> = ring.clone();
        let engine = Engine::with_global_sink(sink.clone());
        ...
        let err = register(&deps, RegisterRequest {
            secret_data_type: None,
            object_type: ObjectType::PublicKey,
            attributes: vec![Attribute::CryptographicUsageMask(UsageMask::VERIFY)],
            managed_object: Some(KeyBlock {
    ...
    /// Negative: an unsupported KeyFormatType (PKCS_1 — RSA-specific,
    /// doesn't apply to EC keys) must fail cleanly, not silently ignore
    /// the format and misparse the bytes.
    #[test]
    fn register_ecdsa_public_key_wrong_format_type_fails_0x10() {
        use softhsmrustv3::native::session;

        let _g = crate::ops::helpers::engine_lock();
        let _ = session::finalize();
        session::init().expect("engine init");
        let sess = session::bootstrap_default_token(0, "so-pin", "user-pin", "wpr2-register-ec-fmt")
            .expect("bootstrap session");
        let ring = Arc::new(RingSink::new(64));
        let sink: Arc<dyn AuditSink> = ring.clone();
        let engine = Engine::with_global_sink(sink.clone());
        ...
        let err = register(&deps, RegisterRequest {
            ...
            managed_object: Some(KeyBlock {
                ...
                cryptographic_algorithm: KmipAlgorithm::Ecdsa,
                cryptographic_length: 256,
                key_value: external_kp.public_key_der(),
                key_wrapping_data: None,
            }),
            protection_storage_masks: None,
            certificate_payload: None,

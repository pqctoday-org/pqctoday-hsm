...
//!
//! Per the 2026-06-07 workflow decision, v0.1 shipped FIPS-certified
//! algorithms only (ML-KEM / ML-DSA / SLH-DSA / classical
//! RSA-ECDSA-AES-HMAC). That posture has since been superseded for two of
//! the originally-parked NIST Round-4 candidates: FrodoKEM (BSI
//! TR-02102-1 §2.4.1) and Classic McEliece (§2.4.2, all 10 parameter
//! sets, implementation plan 2026-09-08) are both exposed here now, per
//! BSI's own recommendation. Falcon, HQC, BIKE, and XMSS remain parked
//! indefinitely — they have reserved codepoints in
//! `pkcs11-mech-manifest.json` but are not exposed here.
//!
...
pub enum KmipAlgorithm {
    ...
    // KMIP's own `CryptographicAlgorithm` enum has REAL OASIS codepoints
    // for exactly three points on the McEliece family (0x34 generic, 0x35 =
    // McEliece-6960119, 0x36 = McEliece-8192128 — confirmed directly in
    // `spec/oasis-kmip-3.0/kmip-spec-3.0-tags-enums.json`). `0x34` was kept
    // as `ClassicMcEliece6688128` (not renamed to a "generic McEliece"
    // variant) when this engine implemented only BSI's Category-5 pick —
    // it is on the wire and in the hub's `kmipMeta.ts`/`codepointTable.ts`,
    // so renaming it now (all-10-parameter-sets implementation plan,
    // 2026-09-08) would be a silent wire break. The other 9 parameter sets
    // are separate variants, not a shared variant plus an attribute
    // (unlike `Ecdh`'s one-variant-many-curves shape) — the CKP-table-
    // ordered PKCS#11 side (§3.1 of that plan) already treats each of the
    // 10 as fully independent, and there's no natural "one wire value + a
    // disambiguating field" split across such a heterogeneous family.
    ClassicMcEliece6688128, // 0x00000034 (OASIS, generic "McEliece")
    /// BSI TR-02102-1 §2.4.2, NIST Category 5. Real OASIS codepoint.
    ClassicMcEliece6960119, // 0x00000035 (OASIS)
    /// BSI TR-02102-1 §2.4.2, NIST Category 5. Real OASIS codepoint.
    ClassicMcEliece8192128, // 0x00000036 (OASIS)
    ...
    // ── BSI TR-02102-1 §2.4.2 — remaining Classic McEliece parameter sets
    // (implementation plan 2026-09-08, §3.2) ────────────────────────────
    // The 7 parameter sets with no OASIS codepoint of their own (0x34/35/36
    // above cover 6688128/6960119/8192128) — vendor/extension range,
    // continuing after `CompositeMlDsa65EcdsaP384Sha512` (0x8000006d), same
    // spec-sanctioned §11.12 convention as the FrodoKEM/HSS/composite-sig
    // entries above. Order matches the CKP_CLASSIC_MCELIECE_* table
    // (size-ascending, non-`f` before its `f` sibling).
    ClassicMcEliece348864,    // 0x8000006e (vendor/extension — non-standard)
    ClassicMcEliece348864F,   // 0x8000006f (vendor/extension — non-standard)
    ClassicMcEliece460896,    // 0x80000070 (vendor/extension — non-standard)
    ClassicMcEliece460896F,   // 0x80000071 (vendor/extension — non-standard)
    ClassicMcEliece6688128F,  // 0x80000072 (vendor/extension — non-standard)
    ClassicMcEliece6960119F,  // 0x80000073 (vendor/extension — non-standard)
    ClassicMcEliece8192128F,  // 0x80000074 (vendor/extension — non-standard)
}
...
impl KmipAlgorithm {
    ...
    pub const fn to_wire_value(self) -> u32 {
        ...
        match self {
            ...
            ClassicMcEliece6960119 => 0x00000035,
            ClassicMcEliece8192128 => 0x00000036,
            ...
            ClassicMcEliece348864    => 0x8000006e,
            ClassicMcEliece348864F   => 0x8000006f,
            ClassicMcEliece460896    => 0x80000070,
            ClassicMcEliece460896F   => 0x80000071,
            ClassicMcEliece6688128F  => 0x80000072,
            ClassicMcEliece6960119F  => 0x80000073,
            ClassicMcEliece8192128F  => 0x80000074,
        }
    }
    ...
    pub const fn from_wire_value(v: u32) -> Option<Self> {
        ...
        Some(match v {
            ...
            0x00000035 => ClassicMcEliece6960119,
            0x00000036 => ClassicMcEliece8192128,
    ...
    pub const fn to_pkcs11_mech(self, op: PkcsOp) -> Option<CkMechanismType> {
        ...
        match (self, op) {
            ...
            // ── Classic McEliece (BSI TR-02102-1 §2.4.2) — all 10 parameter
            // sets share one PKCS#11 mechanism pair; the variant is carried
            // as CKA_PARAMETER_SET (same convention as ML-KEM/SLH-DSA
            // above), not a separate mechanism per set.
            (ClassicMcEliece348864 | ClassicMcEliece348864F
                ...
                | ClassicMcEliece8192128 | ClassicMcEliece8192128F, KeyGen)
                => Some(CKM_PQCTODAY_CLASSIC_MCELIECE_KEY_PAIR_GEN),
            (ClassicMcEliece348864 | ClassicMcEliece348864F
                | ClassicMcEliece460896 | ClassicMcEliece460896F
                | ClassicMcEliece6688128 | ClassicMcEliece6688128F
                | ClassicMcEliece6960119 | ClassicMcEliece6960119F
                | ClassicMcEliece8192128 | ClassicMcEliece8192128F, Encrypt | Decrypt)
    ...
    pub const fn spec_name(self) -> &'static str {
        ...
        match self {
            ...
            // "f" suffix matches liboqs's own algorithm names and the hub
            // playground (implementation plan §3.2).
...
mod tests {
    ...
    fn all_algos() -> &'static [KmipAlgorithm] {
        ...
        &[
            ...
            ClassicMcEliece6688128, ClassicMcEliece6960119, ClassicMcEliece8192128,
            ...
            ClassicMcEliece348864, ClassicMcEliece348864F,
            ClassicMcEliece460896, ClassicMcEliece460896F,
            ClassicMcEliece6688128F, ClassicMcEliece6960119F, ClassicMcEliece8192128F,
        ]
    }
...
mod standard_codepoint_tests {
    ...
    /// `McEliece` value (0x34): the spec's own 6960119/8192128 codepoints
    /// (0x35/0x36) are now used by `ClassicMcEliece6960119`/`8192128`
    /// (implementation plan 2026-09-08, §3.2 — the engine now implements all
    /// 10 parameter sets, so those two real codepoints are no longer
    /// unclaimed); 6688128 itself has no spec-assigned codepoint of its own
    /// and was already on the wire on 0x34 before that plan, so it stays
    /// there rather than moving (would be a silent wire break) — the other 7
    /// parameter sets sit in the Extensions range instead (§3.2 table).
    #[test]
    fn no_invented_codepoints_in_the_standard_range() {
        ...
        assert_eq!(KmipAlgorithm::ClassicMcEliece6960119.to_wire_value(), 0x0000_0035);
        assert_eq!(KmipAlgorithm::ClassicMcEliece8192128.to_wire_value(), 0x0000_0036);
        for f_variant in [
            ...
            assert!(
                f_variant.to_wire_value() >= EXTENSIONS,
                "{} has no §11.12 value, so it must sit in the Extensions range",
                f_variant.spec_name()
            );
        }
        assert!(

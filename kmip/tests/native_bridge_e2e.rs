...
fn hss_register_sign_verify_against_real_engine() {
    ...
    let sig1 = sign(
        &deps,
        SignRequest { uid: priv_uid.clone(), data: b"message one".to_vec(), cryptographic_parameters: None , init_indicator: None, final_indicator: None, correlation_value: None },
        &AuthContext::open(),
        "hss-e2e-sign-1",
    )
    .unwrap();
    ...
    let sig2 = sign(
        &deps,
        SignRequest { uid: priv_uid.clone(), data: b"message two".to_vec(), cryptographic_parameters: None , init_indicator: None, final_indicator: None, correlation_value: None },
        &AuthContext::open(),
        "hss-e2e-sign-2",
    )
    .unwrap();
...
fn k3_register_rsa_public_key_accepts_both_pkcs1_and_pkcs8_der() {
    ...
    let sig = sign(
        &deps,
        SignRequest { uid: priv_uid, data: message.clone(), cryptographic_parameters: None , init_indicator: None, final_indicator: None, correlation_value: None },
        &AuthContext::open(),
        "k3-sign",
    )

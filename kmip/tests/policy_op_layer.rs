...
fn try_create_key_pair(
    ...
    for (k, v) in custom {
        common.push(Attribute::Custom { vendor: None, name: (*k).to_string(), value: pqctoday_kmip::kmip30::CustomAttributeValue::Text((*v).to_string()) });
    }
    create_key_pair(
        deps,
...
fn try_create_sym(
    ...
    for (k, v) in custom {
        t.push(Attribute::Custom { vendor: None, name: (*k).to_string(), value: pqctoday_kmip::kmip30::CustomAttributeValue::Text((*v).to_string()) });
    }

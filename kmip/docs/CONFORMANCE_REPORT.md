...
| ~~**G11**~~ | **MOSTLY CLOSED 2026-09-07** | Done: `Interop` `Reset` (0x03) and the reserved `"*"` identifier are decoded, and Interop is now **refused unless `--enable-interop` is passed** — §6.1.32 says it "SHALL NOT be available in a production server" and it had been unconditionally available (172 corpus transcripts exercise it, so the replay was re-run to prove the gate holds). Client Correlation Value (§9.7) is decoded and logged, as "the server SHOULD log this information" requires. **Still open, all decisions rather than implementations:** four Cryptographic Algorithm codepoints (`EC`/`X25519`/`X448` are modelled as curves not algorithm values, `XMSS` is engine-supported but unnameable, and Classic McEliece sits on the generic `McEliece` codepoint because CSD02 names 6960119/8192128 but not 6688128); the `Object Group` attribute, absent from the CSD02 tag table and replaced by `Group Link`, but still 44 references including a Locate filter, so removing it changes search behaviour; the `Unique Identifier` enumeration (1 of 21, forward-compatibility only); which of the 8 credential types to support (2 authenticate today — Username-and-Password, and Ticket via Login); and the HTTPS/XML/JSON message encodings (§10.2, Profiles §5.3–5.5), which are the real barrier to Complete Server and a programme in themselves. | various |

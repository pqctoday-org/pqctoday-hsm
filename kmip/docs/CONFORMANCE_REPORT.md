...
**Status:** nine of the eleven are now closed or mostly closed (struck through below), each with a
test that fails when the fix is removed. Four carry a named remainder, stated in their row rather
than quietly folded into "closed": §4.6 Certificate Attributes and §4.14 Credential Type (G2),
`Query Profiles` advertisement (G5), streaming for the signature/digest operations (G6), and
per-vendor keying of custom attributes at rest (G9). G4 is the accepted DSA deviation and G11 is
untouched. Remediation plan:
...
| ~~**G5**~~ | **MOSTLY CLOSED 2026-09-06** | All 15 Query Functions answer; the nine missing ones previously failed the WHOLE message. Extension List/Map now disclose `PQCToday-SharedSecret` (0x540001), which a conformant client could otherwise find only by reading this repository. **Still open:** `Query Profiles` returns an empty list while this report claims Baseline Server — the standing K13 judgement about which profiles to formally advertise. | `ops/query.rs` |
| ~~**G6**~~ | **MOSTLY CLOSED 2026-09-06** | Multi-part `Decrypt` implemented — it previously returned **Success with wrong plaintext**, the only defect here that produced a wrong answer rather than an omission. **Still open:** Sign / Signature Verify / MAC / MAC Verify / Hash carry no multi-part fields and get single-shot semantics. | `ops/decrypt.rs` |
...
| ~~**G9**~~ | **MOSTLY CLOSED 2026-09-06** | Vendor Identification now survives decode and is echoed instead of being replaced with `"x"`. **Still open:** `custom_attributes` is keyed by name alone in the store, so two vendors using one name still collide at rest — a data-model migration, deliberately not bundled here. | `kmip30/wire.rs` |
